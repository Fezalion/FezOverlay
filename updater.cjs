...
if (!fs.existsSync(envPath)) {
  ...
  console.log(
    ".env file was missing and has been created with default LASTFM_API_KEY."
  );
}
    ...
    https
      .get(
        ...
        (response) => {
          ...
          if (totalBytes > 0) {
            console.log(
              `File size: ${(totalBytes / 1024 / 1024).toFixed(2)} MB`
            );
          }
            ...
            for (const file of files) {
              ...
              if (fs.statSync(filePath).isDirectory()) {
                ...
                if (fs.existsSync(subIndexPath)) {
                  console.log(
                    `✓ index.html found in subfolder: ${file}/index.html`
                  );
...
function main() {
  ...
  getLatestRelease((err, release) => {
    ...
    if (!distExists) {
      ...
      console.log("New version available.Cleaning then updating...");
        ...
        (err) => {
          if (err) {
            ...
            try {
              ...
              console.error(
                "✗ Failed to replace fezoverlay.exe:",
                renameErr.message
              );
              hasError = true;
            }
    ...
    if (updaterAsset) {
      ...
      downloadFile(
        ...
        path.join(baseDir, updaterName + ".new"),
        (err) => {
          if (err) {
    ...
    if (distAsset) {
      ...
      console.log(
        `[${stepNumber}/${totalDownloads}] Updating application files...`
      );
      downloadFile(
        distAsset.browser_download_url,
        path.join(baseDir, distZipName),
        (err) => {
          if (err) {
          ...
          try {
            ...
            console.log(
              `Zip file size: ${(zipStats.size / 1024 / 1024).toFixed(2)} MB`
            );
          } catch (statErr) {
